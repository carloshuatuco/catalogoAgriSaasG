...
// Correos con acceso permanente (propietarios del SaaS)
...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Verificar licencia en magistral-afiliados
      ...
      if (tieneAcceso) {
        ...
        await logout();
        setError(
          "No tienes una licencia activa para Catálogos Virtuales. " +
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 flex-col px-4 text-sans">
      <div className="w-full max-w-sm bg-white p-8 rounded-2xl shadow-xl border border-gray-100 text-center">
        ...
        {error && (
          <div className="bg-red-50 text-red-600 p-3 rounded-lg text-xs font-medium mb-4 leading-relaxed">
            {error}
          </div>
        )}
        ...
        >
          ...
          <span className="text-sm">{loading ? "Verificando acceso..." : "Ingresar con cuenta de Google"}</span>
        </button>
